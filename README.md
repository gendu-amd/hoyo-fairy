...
覆盖 **首页 / 热门 / 排行榜 / 搜索 / 播放页推荐 / 动态 / 评论区**，兼容 **Edge / Chrome / Firefox**（装 Tampermonkey 即可，无需单独扩展）。当前为 **pre-release v0.0.5**。
...
2. **最快上手**：「工具」里点预置规则库按钮（如 `+ 库洛系(鸣潮/库洛)`），规则立即灌入对应黑名单生效（预置只是一键灌词，灌完可在「黑名单」页增删）。
...
- [ ] 反绕过匹配（隐形字符剔除 + 分隔符模糊匹配）
- [ ] 营销号识别（高播放 + 低点赞率启发式，阈值待真机调优）
- [ ] 风控熔断（联网触发风控时自动退避）
- [ ] 预置规则库 v2（按大类分组 / 多维度 / 一键叠加）
- [ ] 评论命中折叠（一行灰条，点击展开真评论）
